...
起動すると **本棚画面** が表示され、各 `epNN`（＝1冊の作品）が背表紙として
並びます。背表紙をタップするとその作品の章が読み込まれてリーダーが開き、
リーダー左上の「← 本棚」で棚に戻れます。章データは **本を開いたときに
初めて読み込まれる**（遅延ロード）ので、起動時はメタ情報だけで軽量です。

背表紙の見た目は `episode.js` の `book` から決まります（任意で調整可能）:

```js
registerEpisode('ep01', {
  label: '蒼のままで',
  book: {
    title: '蒼のままで', author: '朝倉雪菜', /* …既存… */
    shelf: { spine: '#1b3a5c', accent: '#e8f0f8' }  // 任意：背表紙色／箔文字色
  }
});
```

`shelf` を省略すると、背表紙色はタイトルから自動生成されます。
